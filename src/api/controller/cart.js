...
module.exports = class extends Base {
  ...
  async getCart() {
    ...
    const cartList = await this.model('cart').where({user_id, session_id: 1, wxapp_id}).select();
    // 获取购物车统计信息
    let goodsCount = 0;
   ...
   */
  async addAction() {
    const goods_id = this.post('goodsId');
    const product_id = this.post('productId');
    const number = this.post('number');
    const wxapp_id = this.header('wxapp_id');

    // 判断商品是否可以购买
    const goodsInfo = await this.model('goods').where({id: goods_id, wxapp_id}).find();
    if (think.isEmpty(goodsInfo) || goodsInfo.is_delete === 1) {
      return this.fail(400, '商品已下架');
    }

    // 取得规格的信息,判断规格库存
    const productInfo = await this.model('product').where({goods_id, id: product_id, wxapp_id}).find();
    if (think.isEmpty(productInfo) || productInfo.goods_number < number) {
      return this.fail(400, '库存不足');
    }

    // 判断购物车中是否存在此规格商品
    const cartInfo = await this.model('cart').where({goods_id, product_id, wxapp_id}).find();
    if (think.isEmpty(cartInfo)) {
      ...
      if (!think.isEmpty(productInfo.goods_specification_ids)) {
        goodsSpecificationValue = await this.model('goods_specification')
          .where({
            goods_id,
            wxapp_id,
      ...
      const cartData = {
        ...
        goods_specifition_name_value: goodsSpecificationValue.join(';'),
        goods_specifition_ids: productInfo.goods_specification_ids,
        checked: 1
      };

      await this.model('cart').thenAdd(cartData, {product_id});
    } else {
      // 如果已经存在购物车中，则数量增加
  ...
  async updateAction() {
    const wxapp_id = this.header('wxapp_id');
    const goods_id = this.post('goodsId');
    const product_id = this.post('productId'); // 新的product_id
    ...
    const cartInfo = await this.model('cart').where({id: id, wxapp_id}).find();
    // 只是更新number
    if (cartInfo.product_id === product_id) {
      await this.model('cart').where({id: id, wxapp_id}).update({
        number: number
      });
    ...
    const newCartInfo = await this.model('cart').where({goods_id, product_id, wxapp_id}).find();
    if (think.isEmpty(newCartInfo)) {
      ...
      if (!think.isEmpty(productInfo.goods_specification_ids)) {
        goodsSepcifition = await this.model('goods_specification')
          ...
          .where({
            'nideshop_goods_specification.wxapp_id': wxapp_id,
            'nideshop_goods_specification.goods_id': goods_id,
      ...
      await this.model('cart').where({id: id, wxapp_id}).update(cartData);
    } else {
      ...
      await this.model('cart').where({id: newCartInfo.id, wxapp_id}).delete();
  ...
  async checkedAction() {
    const wxapp_id = this.header('wxapp_id');
    let productIds = this.post('productIds').toString();
    ...
    await this.model('cart').where({product_id: {'in': productIds}, wxapp_id}).update({checked: parseInt(isChecked)});
  ...
  async deleteAction() {
    ...
    if (!think.isString(productIds)) {
      return this.fail('删除出错');
    }

    productIds = productIds.split(',');

    await this.model('cart').where({product_id: {'in': productIds}, wxapp_id}).delete();
  ...
  async checkoutAction() {
    ...
    if (addressId) {
      checkedAddress = await this.model('address').where({is_default: 1, user_id: this.getLoginUserId(), wxapp_id}).find();
    } else {
      checkedAddress = await this.model('address').where({id: addressId, user_id: this.getLoginUserId(), wxapp_id}).find();
    }
    ...
    // 获取可用的优惠券信息，功能还示实现
    const couponList = await this.model('user_coupon').where({wxapp_id}).select();
