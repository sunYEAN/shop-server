...
module.exports = class extends Base {
  async indexAction() {
    ...
    // 获取顶级分类列表
    const supCategoryList = await caregory
      .where({parent_id: 0, name: ['<>', '推荐'], is_delete: 0})
      .select();

    // 分类列表
    let newCategoryList = [];
    for (const sup of supCategoryList) {
      const childCategoryIds = await this.model('category').where({parent_id: sup.id}).getField('id', 100);
